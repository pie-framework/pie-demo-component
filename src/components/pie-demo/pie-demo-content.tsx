...
export class PieDemoContent {
    ...
    componentDidUpdate() {
        //todo keep updating the configSettings
        if (this.pieAuthor) {
    ...
    getProperController = (model) => {
        ...
        if ("markup" in this.config) {
            ...
            const tag = Object.keys(this.config.elements).find(key => key === divA.querySelector(`[id="${model.id}"]`).tagName)

            return {
                controller: this.pieControllers[this.config.elements[tag]],
                tag
            }
        }
    ...
    watchConfig(newConfig) {
        console.log("[watchConfig] ", newConfig);

        Object.values(newConfig.elements || {}).map((packageNAME: string) => {
            let packageCopy = packageNAME;
            let pieName = packageCopy
                .substr(packageCopy.lastIndexOf("/") + 1, packageCopy.length)
                .split("@")[0];
    ...
    async updatePieModelUsingController(model, env) {
    ...
    setMode(mode) {
        ...
        if ("models" in this.config) {
            console.log('updating all models this.session = ', this.session);
            console.log('updating all models this.env = ', this.env);
        ...
        return (
            ...
            >
                ...
                {!isCollapsed && (
                    ...
                    >
                        {Object.keys(this.scores).map(scoreKey => (
                            <div class={classnames("score-holder", {visible: this.env["mode"] === "evaluate"})}>
                                <div class="score">
                                    <span>Score for session ${scoreKey}: </span> {this.scores[scoreKey].score}
                                </div>
                                <pre>{this.scores[scoreKey].json}</pre>
                            </div>
                        ))}
                        <div
